...
function ct_tribes_register_theme_page() {
	add_theme_page( sprintf( esc_html__( '%s Dashboard', 'tribes' ), wp_get_theme( get_template() ) ), sprintf( esc_html__( '%s Dashboard', 'tribes' ), wp_get_theme( get_template() ) ), 'edit_theme_options', 'tribes-options', 'ct_tribes_options_content', 'ct_tribes_options_content' );
}
...
function ct_tribes_options_content() {
	...
	<div id="tribes-dashboard-wrap" class="wrap">
		<h2><?php printf( esc_html__( '%s Dashboard', 'tribes' ), wp_get_theme( get_template() ) ); ?></h2>
		<?php do_action( 'ct_tribes_theme_options_before' ); ?>
		<div class="content-boxes">
			<div class="content content-support">
				<h3><?php esc_html_e( 'Get Started', 'tribes' ); ?></h3>
			...
			<?php if ( !function_exists( 'ct_tribes_pro_init' ) ) : ?>
				<div class="content content-premium-upgrade">
					<h3><?php printf( esc_html__( '%s Pro', 'tribes' ), wp_get_theme( get_template() ) ); ?></h3>
					<p><?php printf( esc_html__( 'Download the %s Pro plugin and unlock custom colors, new layouts, sliders, and more', 'tribes' ), wp_get_theme( get_template() ) ); ?>...</p>
					<p>
						<a target="_blank" class="button-primary"
						   href="https://www.competethemes.com/tribes-pro/"><?php esc_html_e( 'See Full Feature List', 'tribes' ); ?></a>
					</p>
				</div>
			<?php endif; ?>
			<div class="content content-review">
				<h3><?php esc_html_e( 'Leave a Review', 'tribes' ); ?></h3>
				<p><?php printf( esc_html__( 'Help others find %s by leaving a review on wordpress.org.', 'tribes' ), wp_get_theme( get_template() ) ); ?></p>
				<a target="_blank" class="button-primary" href="https://wordpress.org/support/theme/tribes/reviews/"><?php esc_html_e( 'Leave a Review', 'tribes' ); ?></a>
			</div>
			...
			<div class="content content-delete-settings">
				<h3><?php esc_html_e( 'Reset Customizer Settings', 'tribes' ); ?></h3>
				<p>
				...
				<form method="post">
					...
					<p>
						...
						<?php submit_button( esc_html__( 'Reset Customizer Settings', 'tribes' ), 'delete', 'delete', false ); ?>
					</p>
				</form>
			</div>
		</div>
