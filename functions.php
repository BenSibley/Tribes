...
if ( ! function_exists( 'ct_tribes_filter_read_more_link' ) ) {
	function ct_tribes_filter_read_more_link() {
		global $post;
		...
		if ( empty( $read_more_text ) ) {
			$output .= '<div class="more-link-wrapper"><a class="more-link" href="' . esc_url( get_permalink() ) . '">' . __( 'Continue reading', 'tribes' ) . '<span class="screen-reader-text">' . esc_html( get_the_title() ) . '</span></a></div>';
		} else {
...
if ( ! function_exists( 'ct_tribes_filter_manual_excerpts' ) ) {
	function ct_tribes_filter_manual_excerpts( $excerpt ) {
		$excerpt_more = '';
		if ( has_excerpt() ) {
			$excerpt_more = ct_tribes_filter_read_more_link();
		}
